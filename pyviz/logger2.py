...
class AnkleLoggerGUI(ctk.CTk):
    ...
    def setup_ui(self):
        """Constructs the two-panel GUI."""
        self.grid_columnconfigure(0, weight=1) 
        self.grid_columnconfigure(1, weight=2) 
        self.grid_rowconfigure(0, weight=1)

        # --- LEFT PANEL (Controls) ---
        # FIX: Changed to CTkScrollableFrame to prevent buttons from falling off-screen
        ...
        for i in range(5):
            ctk.CTkButton(btn_frame, text=str(i), width=35, 
                          command=lambda x=i: self.repl.motor_state(x)).pack(side="left", padx=2)
    ...
    def set_pid(self):
        try:
            # FIX: Added network transmission delays to prevent queue overwriting in the RTOS
    ...
    def update_pid_fields(self, p, i, d):
        """Callback triggered by the background thread. Pushes UI updates to the main loop."""
        self.after(0, self._apply_pid_fields, p, i, d)

    def _apply_pid_fields(self, p, i, d):
        """Thread-safe update of the CTkEntry widgets."""
    ...
    def toggle_recording(self, enable):
        ...
        print(f"Recording controls {'enabled' if enable else 'disabled'}.")
    ...
    def start_recording(self):
        ...
        if not name:
            ...
            return
        self.recorder.start_trial(name)
