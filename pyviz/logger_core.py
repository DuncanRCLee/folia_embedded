...
# TCP Port configuration needed for DeviceREPL
TCP_PORT = 8080

# Protobuf descriptor setup needed for TrialRecorder
proto_fds = descriptor_pb2.FileDescriptorSet()
gen.Packet_pb2.DESCRIPTOR.CopyToProto(proto_fds.file.add())
packet_descriptor = gen.Packet_pb2.Packet.DESCRIPTOR
...
class DeviceREPL:
    ...
    def _parse_pid_sync(self, text: str):
        """Parses [PID_SYNC] P:4.000 I:0.100 D:0.200 and triggers GUI callback."""
        try:
            parts = text.split()
            p_val = parts[1].split(':')[1]
            i_val = parts[2].split(':')[1]
            d_val = parts[3].split(':')[1]
            if self.pid_callback:
                self.pid_callback(p_val, i_val, d_val)
        except Exception as e:
            print(f"Failed to parse PID sync: {e}")

    def request_pid(self):
        """Requests the current PID values from the device."""
        return self.send("R")
